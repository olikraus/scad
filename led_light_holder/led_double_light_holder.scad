...
module holder(is_screw_hole=true, is_holder=true, ry=0) {
  difference() {
    CenterCube([outer_width, total_len, outer_height]);
    if ( is_holder == true ) {
        translate([0,0,base_height+0.01])
        rotate([0,ry,1])
        CenterCube([inner_width, 2*total_len, inner_height],
            ChamferBottom=3.6, ChamferTop=3.6);
        translate([0,0,base_height+inner_height/2])
        CenterCube([inner_width-5, 2*total_len, inner_height]);
    }
    else {
        translate([0,0,base_height+0.01])
        CenterCube([outer_width+0.01, 2*total_len, inner_height*2],
            ChamferBottom=3.6);
    }

    if ( is_screw_hole == true ) {   
